            ...
            if(mTextFile != null) {
                ...
                if(refreshBinds && mTextFile.text.Length > 0) {
                    ...
                    foreach(var node in json) {
                        ...
                        loadBinds.Add(new InputManager.Bind() { action = action, control = control, axisInvert = axisInvert, deadZone = deadZone, forceRaw = forceRaw, keys = inpkeys });
                    }
                    //
                ...
                for(int i = 0; i < mBinds.Length; i++) {
                    ...
                    if(mBinds[i].bind.control == InputManager.Control.Axis) {
                        ...
                        mBinds[i].bind.axisInvert = EditorGUILayout.Toggle("Invert", mBinds[i].bind.axisInvert);
                    }
